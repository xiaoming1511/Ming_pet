        ...
        <n-drawer v-model:show="sidebarStore.active" :default-width="1100" resizable close-on-esc :max-width="1260"
            ...
            <n-drawer-content title="服务" closable>
                ...
                <n-flex justify="end" class="items-center text-2xl h-12" v-if="ServiceTab">
                    <n-select class="w-64" @update:value="handleUpdateValue" filterable :options="selectPeople"
                        :render-label="renderLabel" :render-tag="renderSingleSelectTag" placeholder="请选择顾客" />
...
const submitOrder = async () => {
    ...
    // 清零动画组件的值
    if (numberAnimationInstRef.value) {
        numberAnimationInstRef.value = 0;
    }
